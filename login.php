...
function belepell() {
   ...
   // $sid=urlencode($sid);
    ...
    if(!$lekerdez=mysql_query($query)) {
        $hiba=true;
        $hibauzenet.='';
        $hibauzenet_prog.='HIBA a beléptetésnél (adatbázis ellenõrzés) - login.php [#12]<br>'.mysql_error();
    }
...
function beleptet() {
    ...
    if(!$lekerdez=@mysql_query($query)) {
        $hiba=true;
        $hibauzenet.='';
        $hibauzenet_prog.='HIBA a beléptetésnél (adatbázis ellenõrzés) - login.php [#75]<br>'.mysql_error();
    }
