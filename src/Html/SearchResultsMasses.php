...
class SearchResultsMasses extends Html
{
    public function __construct()
    {
        ...
        $zeneT = ['g' => 'gitáros', 'o' => 'orgonás', 'cs' => 'csendes', 'na' => 'meghátorazatlan'];
        $korT = ['csal' => 'családos', 'd' => 'diák', 'ifi' => 'ifjúsági', 'na' => 'meghátorazatlan'];
        $ritusT = ['gor' => 'görögkatolikus', 'rom' => 'római katolikus', 'regi' => 'régi rítusú'];
        ...
        if ($mikordatum == $ma) {
            ...
            $napokT = ['vasárnap', 'hétfő', 'kedd', 'szerda', 'csütörtök', 'péntek', 'szombat'];
        ...
        if (!empty($nyelv) || (!empty($zene) && \count($zene) < 3) || (!empty($kor) && \count($kor) < 4)) {
