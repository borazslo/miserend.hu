...
class Church extends \Illuminate\Database\Eloquent\Model {
    ...
    public function remarks() {
        return $this->hasMany(\App\Model\Remark::class)->orderBy('created_at', 'DESC');
    }
    ...
    public function getHoldersAttribute($value) {
        $holders =  \App\Model\ChurchHolder::where('church_id',$this->id)->orderBy('status')->orderBy('updated_at','desc')->get()->groupBy('status');
        return $holders;
    }
    
    public function getLinksAttribute($value) {
        $links =  $this->hasMany(\App\Model\ChurchLink::class)->get();
        return $links;
    }
    ...
    public function getJelzesAttribute() {
      ...
      public function getRemarksiconAttribute() {
		 ...
		 $allapotok = \App\Model\Remark::where('church_id',$this->id)->groupBy('allapot')->pluck('allapot')->toArray();
    ...
    function getLocationAttribute($value) {
        ...
        $tags = collect(
            \App\Model\OSMTag::where('osmtype',$this->osmtype)
	...
	function getOsmAttribute($value) {	
		...
		$osm = \App\Model\OSM::where('osmtype',$this->osmtype)
                ->where('osmid',$this->osmid)                
                ->first();
		if(!$osm) $osm = new \App\Model\OSM(['osmid'=>$this->osmid, 'osmtype' => $this->osmtype]);
    ...
    public function MgetReligious_administration() {
        ...
        $this->religious_administration->diocese = new \App\Diocese();
        $this->religious_administration->diocese->getByChurchId($this->id);
        $this->religious_administration->deaconry = new \App\Deaconry();
    ...
    function checkWriteAccess($_user) {
        ...
        if(\App\Model\ChurchHolder::where('church_id',$this->id)->where('user_id',$_user->uid)->where('status','allowed')->first())
            $access = true;
    ...
    function MdownloadOSMBoundaries() {
        ...
        foreach($overpass->jsonData->elements as $element) {
            $boundary = \App\Model\Boundary::firstOrNew(['osmtype' => $element->type, 'osmid' => $element->id]);
    ...
    function MmigrateBoundaries() {
        ...
        $tmp = $this->boundaries()
                ...
                ->get()->toArray();        
        if($tmp == array()) {
            $boundary = \App\Model\Boundary::firstOrNew(['boundary' => 'religious_administration', 'denomination' => $this->denomination, 'admin_level' => 6, 'name' => $_egyhazmegyek[$this->egyhazmegye]->nev]);
            $boundary->save(); 
            $this->boundaries()->attach($boundary->id);
        }
        ...
        $tmp = $this->boundaries()
                ...
                ->get()->toArray();
        if($tmp == array()) {
            $boundary = \App\Model\Boundary::firstOrNew(['boundary' => 'religious_administration', 'denomination' => $this->denomination, 'admin_level' => 7, 'name' => $_espereskeruletek[$this->espereskerulet]->nev]);
            $boundary->save();
            $this->boundaries()->attach($boundary->id);            
        }
        ...
        $tmp = $this->boundaries()
                ...
                ->get()->toArray();
        if($tmp == array()) {
            $boundary = \App\Model\Boundary::firstOrNew(['boundary' => 'administrative', 'admin_level' => 2, 'name' => $_orszagok[$this->orszag]->nev]);
            $boundary->save();
            $this->boundaries()->attach($boundary->id);            
        }
        ...
        if($tmp == array()) {
            if(isset($_megyek[$this->megye])) {
                $boundary = \App\Model\Boundary::firstOrNew(['boundary' => 'administrative', 'admin_level' => 6, 'name' => $_megyek[$this->megye]->nev." megye"]);
                $boundary->save();
                $this->boundaries()->attach($boundary->id);            
            }
        }        
        ...
        $tmp = $this->boundaries()
                ...
                ->get()->toArray();
        if($tmp == array()) {
            $boundary = \App\Model\Boundary::firstOrNew(['boundary' => 'administrative', 'admin_level' => 8, 'name' => $this->varos]);
            $boundary->save();
            $this->boundaries()->attach($boundary->id);  
        }
    ...
    public function delete() {
        ...
        \App\Model\ChurchHolder::where('church_id',$this->id)->delete();
        \App\Model\Favorite::where('tid',$this->id)->delete();
        \App\Model\ChurchLink::where('church_id',$this->id)->delete();
