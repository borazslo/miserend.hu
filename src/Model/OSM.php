...
{
    protected $table = 'osm';
    protected $fillable = ['osmid', 'osmtype'];
    protected $appends = ['tagList', 'name'];
    ...
    {
        return $this->hasMany(OSMTag::class, 'osmid', 'id')->orderBy('name');
    }

    public function churches()
    {
        return $this->belongsToMany(Church::class, 'lookup_church_osm', 'osm_id', 'church_id');
    }
    ...
    {
        $return = [];
        $tags = OSMTag::where('osmtype', $this->osmtype)
    ...
    {
        return $this->belongsToMany(self::class, 'lookup_osm_enclosed', 'osm_id', 'enclosing_id');
    }

    // Return OSM elements that is enclosed by this element. This element encloses the returning ones.
    public function enclosed()
    {
        return $this->belongsToMany(self::class, 'lookup_osm_enclosed', 'enclosing_id', 'osm_id');
    }
