...
class Josm extends Html
{
    public function __construct($path)
    {
        if (isset($_REQUEST['update'])) {
            ...
            $job = \App\Legacy\Model\Cron::where('class', '\App\Legacy\OSM')->where('function', 'checkUrlMiserend')->first();
            $job->run();
        }
        ...
        $this->cron = \App\Legacy\Model\Cron::where('class', '\App\Legacy\OSM')
        ...
        $this->churchesWNoOsm = \App\Legacy\Model\Church::where('ok', 'i')
        ...
        $this->churchesWBadOsm = \App\Legacy\Model\Church::where('ok', 'i')
                ->whereNotIn('id', $goodIDs)
                ->whereNotNull('osmtype')->whereNotNull('osmid')
                ->orderBy('orszag')->orderBy('megye')->orderBy('varos')->orderBy('nev')
                ->get();

        $this->churchesWBad = \App\Legacy\Model\Church::where('ok', 'i')
                ->whereNotIn('id', $goodIDs)
                ->get();
    }
    ...
    public function checkOsmElements($elements)
    {
        ...
        foreach ($elements as $element) {
            ...
            if (!isset($match[2])) {
                ...
                $church = \App\Legacy\Model\Church::find($match[2]);
                if ($church) {
