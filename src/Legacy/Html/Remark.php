...
class Remark extends Html
{
    ...
    public function add(Request $request): Response
    {
        ...
        if ($request->getMethod() === 'POST') {
            $remark = new \App\Legacy\Model\Remark();
            ...
            $megbizhato = \App\Legacy\Model\Remark::select('megbizhato')->where('email', $remark->email)->orderBy('created_at', 'desc')->limit(1)->first();
            if ($megbizhato) {
...
switch ($this->action) {
    case 'list':
        ...
        if (\App\Legacy\Request::Simpletext('remark') == 'modify') {
            ...
            $remark->allapot = \App\Legacy\Request::Simpletext('state');
            $remark->admindatum = date('Y-m-d H:i:s');

            $remark->appendComment(\App\Legacy\Request::Text('adminmegj'));
            $remark->save();
