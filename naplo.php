...
function teendok($tid) {
    ...
    if (!empty($templom['ismertnev']))
        $templom['ismertnev'] = "(" . $templom['ismertnev'] . ")";
    $vars['church'] = $templom;
...
function mod() {
    ...
    if ($remark->id < 1)
        die('Attól még, hogy szám az rid, még nem biztos, hogy létezik.');

    if ($adminmegj != '') {
        if (!empty($remark->adminmegj))
            $remark->adminmegj.="\n";
    ...
    if ($allapot == 'plebaniara' OR $allapot == 'koszon')
        $allapot = 'j';
