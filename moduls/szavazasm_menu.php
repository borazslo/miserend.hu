...
function szavazas_jobbmenu_add() {
	...
	if(!strstr($szavazott,$fooldal_id)) {
		...
		//(új látogató, új böngészőablakkal új sessiont kap)
		...
		if(($most>($datum+$lejar)) or $datum==0) {
			...
			if($datum>0) //Ha korábban már szavazott, most frissítjük
				mysql_query("update szavazasell set datum='$most' where ip='$ip' and kid='$kid'"); //milyen IP-ről mikor szavazott
			else //Ha még nem szavazott, létrehozzuk az ellenőrzéshez
				mysql_query("insert szavazasell set datum='$most', ip='$ip', kid='$kid'"); //milyen IP-ről mikor szavazott
		}
		else {
			//fél órán belül már szavaztak erről a gépről, de azért az időt frissítjük, újra indul a fél óra
			mysql_query("update szavazasell set datum='$most' where ip='$ip' and kid='$kid'"); //milyen IP-ről mikor szavazott
		}
