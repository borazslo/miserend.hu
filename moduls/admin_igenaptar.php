...
function gondolatokmod() {
    ...
    if ($idoszak != '0' and $idoszak != '')
        $queryT[] = "idoszak='$idoszak'";
    if ($ev != '0' and $ev != '')
        $queryT[] = "ev='$ev'";
    if ($kulcsszo != '' and $kulcsszo != '0')
        $queryT[] = "(intro like '%$kulcsszo%' or gondolat like '%$kulcsszo%' or oszov like '%$kulcsszo%' or ujszov like '%$kulcsszo%' or evang like '%$kulcsszo%')";
    if (is_array($queryT)) {
    ...
    if ($ige > 0)
        ...
        $keres.=$query;
    if (!$lekerdez = mysql_db_query($db_name, $keres))
        echo 'HIBA<br>' . $keres . '<br>' . mysql_error();
...
//Módosításnál (vagy kiválasztja, vagy kulcsszó alapján keresi
    ...
    if (!$lekerdez = mysql_query($query))
        $kod.= '<p class=hiba>HIBA!<br>' . mysql_error();
...
function szentekmod() {
    ...
    if ($szid != 0) {
        ...
        $kod.= '<p class=alap>';
        if ($min > 0)
            $kod.= " <a href=?m_id=$m_id&m_op=szentekmod&kulcsszo=$kulcsszo&min=$prev$linkveg class=link1>Előző</a>";
        if ($mennyi > $next)
            $kod.= " <a href=?m_id=$m_id&m_op=szentekmod&kulcsszo=$kulcsszo&min=$next$linkveg class=link1>Következő</a>";
    }
...
function naptar($honap, $ev) {
    ...
    foreach ($napok as $nap) {
        if ($nap == 'Vasárnap')
            $tulajdonsag = 'bgcolor=#FFF9F9 class=unnep';
        else
            $tulajdonsag = 'class=link';
    ...
    for ($szamlalo = 0; $szamlalo < (6 * 7); $szamlalo++) {
        ...
        //ünnepek:
        if ($napTomb[mon] == 1 and $napTomb["mday"] == 1) {
            $unnep = 1;
            $katunnep = 1;
            $msg = 'Újév';
        } elseif ($napTomb[mon] == 3 and $napTomb["mday"] == 15) {
            $unnep = 1;
            $msg = 'Nemzeti ünnep';
        } elseif ($napTomb[mon] == 8 and $napTomb["mday"] == 20) {
            $unnep = 1;
            $msg = 'Nemzeti ünnep';
        } elseif ($napTomb[mon] == 10 and $napTomb["mday"] == 23) {
            $unnep = 1;
            $msg = 'Nemzeti ünnep';
        } elseif ($napTomb[mon] == 11 and $napTomb["mday"] == 1) {
            $unnep = 1;
            $katunnep = 1;
            $msg = 'Mindenszentek ünnepe';
        } elseif ($napTomb[mon] == 12 and $napTomb["mday"] == 25) {
            $unnep = 1;
            $katunnep = 1;
            $msg = 'Karácsony';
        } elseif ($napTomb[mon] == 12 and $napTomb["mday"] == 26) {
            $unnep = 1;
            $katunnep = 1;
            $msg = 'Karácsony';
        } else
        ...
        if ($szamlalo < $elsoNapTombje["wday"] || $napTomb["mon"] != $honap) {
            ...
            if ($ujnaptomb[mday] == $napTomb[mday])
                $kiirando += EGYNAP;
            $kod.= "</td>\n";
        }
    }
