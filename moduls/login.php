...
function belepell() {
    ...
    if(!@$lekerdez=mysql_db_query($db_name,$query)) {
        $hiba=true;
        $hibauzenet.='';
        $hibauzenet_prog.='HIBA a beléptetésnél (adatbázis ellenõrzés) - login.php [#20]<br>'.mysql_error();
    }
    if(!$tiltott_IP_T[1]) {
...
function beleptet() {
    ...
    if(!$lekerdez=@mysql_db_query($db_name,$query)) {
        $hiba=true;
        $hibauzenet.='';
        $hibauzenet_prog.='HIBA a beléptetésnél (adatbázis ellenõrzés) - login.php [#88]<br>'.mysql_error();
    }
...
function ellsessid($u_sessid,$u_id) {
    global $db_name;
    $sessid_ok=false;
    //Ellenõrzés, hogy van-e már ilyen sessid
    $query="select uid from session where sessid='$u_sessid'";
    if(!@$lekerdez=mysql_db_query($db_name,$query)) {
        $hiba=true;
        $hibauzenet.='';
        $hibauzenet_prog.='HIBA a beléptetésnél (sessid ellenõrzés) - login.php [#145]<br>'.mysql_error();
    }
    if(@mysql_num_rows($lekerdez)==0) {
        $sessid_ok=true;
    }
