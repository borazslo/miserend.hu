...
function hirek_index() {
	...
	$ma=date('Y-m-d');
	$lejart=date('Y-m-d H:i:s',time()-172800); //2 nap
	$rovat=$_GET['rovat'];

	$min=$_GET['min'];
	if($min<0 or !isset($min)) $min=0;

	$query="select id,nev from rovatkat";
...
//Hírek keresése
	...
	$query="select id,cim,intro,datum,rovatkat from hirek where ok='i' and datum<='$most' and datum>='$lejart' order by datum desc";
	$lekerdez=mysql_db_query($db_name,$query);
	while(list($id,$cim,$intro,$datum,$rovatkat)=mysql_fetch_row($lekerdez)) {
		...
		if($hol>0 and $hol<=6) {
			if($rovattovabbiT[$hol]<2) {//Ha az adott rovatban még nincs meg a két további
				...
				$rovathirekT[$hol]['tovabbi'] .= hirlista_formazo($hir_listaT);			
			}
		}

		if($frisshirek<$maxhir) {
			$frisshirek++;
			$hir_listaT['link']="?hir=$id$linkveg";
			$hir_listaT['hid']=$id;
			$hir_listaT['cim']=$cim;
			$hir_listaT['idopont']="($datumkiir)";
			$hir_listaT['lead']=$intro;
			$hir_listaT['mi']='fhp';
			
			$hirlistaT[] = hirlista_formazo($hir_listaT);			
		}
	}
...
//Rovatfőhírek
	...
	while(list($id,$cim,$intro,$kiemeles_hol,$kiemeles_tol)=mysql_fetch_row($lekerdez)) {
		...
		if(!$rovatT[$kiemeles_hol]) {//Ha az adott rovatban még nincs kiemelt
			...
			$r_cim=str_replace('ő','Ő',$r_cim);
			$rovat['link']="?hir=$id$linkveg";
			$rovat['cim']=$cim;
			$rovat['intro']=$intro;
			$rovat['datum']=$datumkiir;
			$tovabbi=$rovathirekT[$kiemeles_hol]['tovabbi'];
			if(!empty($tovabbi)) $tovabbi="<div class=\"tovabbi\">$tovabbi</div>";

			$tmpl_file = $design_url.'/sablon_rovat.htm';
			$thefile = implode("", file($tmpl_file));
			$thefile = addslashes($thefile);
			$thefile = "\$r_file=\"".$thefile."\";";
			eval($thefile);

			$rovatokT[$kiemeles_hol] .= $r_file;
		}
	}
	...
	$query="select id,cim,intro,aktualis from hirek where ok='i' and datum<='$most' and aktualis!='' $feltetel";
	$lekerdez=mysql_db_query($db_name,$query);
	while(list($id,$cim,$intro,$aktualis)=mysql_fetch_row($lekerdez)) {
		...
		foreach($aktualisT as $datum) {	
			...
			if(!$voltmar[$id]) {
				if($datum>$ma and $datum<$hetvege) {
					...
					$hir_listaT['hid']=$id;
					$hir_listaT['cim']=$cim;
					$hir_listaT['idopont']="($nap)";
					$hir_listaT['lead']=$intro;
					$hir_listaT['mi']='knp';
			
					$aktualisesemenyT[$datum][] = hirlista_formazo($hir_listaT);		
	...
	//Közeli határidők keresése
	$maxhatarido=10;
	$ma=date('Y-m-d');
	$egynap=86400;
	$hetvege=date('Y-m-d',(time()+(7*$egynap)));
	$query="select id,cim,intro,hatarido from hirek where ok='i' and hatarido>='$ma' and hatarido<='$hetvege' order by hatarido limit 0,$maxhatarido";
	$lekerdez=mysql_db_query($db_name,$query);
	while(list($id,$cim,$intro,$hatarido)=mysql_fetch_row($lekerdez)) {
		if($hatarido==$ma) {
			$hataridokiir='';
			...
			$hir_listaT['hid']=$id;
			$hir_listaT['cim']=$cim;
			$hir_listaT['idopont']="(határidő: $nap)";
			$hir_listaT['lead']=$intro;
			$hir_listaT['mi']='kh';
			
			$hataridokT[] = hirlista_formazo($hir_listaT);	
	}

	if(is_array($hataridokT) and count($hataridokT)>0) $hataridok=implode("\n\n",$hataridokT);
	else $hataridok="<span class=alap>a következő napokban nincs határidő</span>";
	...
	*/
	...
	$elotercim="<a href='#'>Van időnk a fájdalomra!</a>";
	$eloterlead="Halálról, szenvedésről, és arról, hogy miért nehéz szembenézni velük...";
	$eloterlink="#";
...
//Főhír keresése -> csak ha nem a szemle rovatban vagyunk!
...
//Kiemelt hírek keresése
	$feltetelnem=" and id!='$id'"; //a főhír nem jelenhet meg kiemeltként is!
...
function hirek_view() {
	...
	if(mysql_num_rows($lekerdez)>0) {
		...
		$honap=alapnyelv("ho$ho");
		$nap=date('j',$datido);
		$cikkdatum="$ev $honap $nap. $ora:$perc";

		if(!empty($halcim)) $alcimkiir="<div class=\"alcim\">$halcim</div>";
		...
		if(!empty($hkulcsszo)) {
			...
			$query="select id,cim,intro,datum from hirek where ok='i' and id!='$id' and datum<='$most' $feltetel order by datum desc limit 0, 10";
			if(!$lekerdez=mysql_db_query($db_name,$query)) echo 'HIBA!<br>'.mysql_error();
			$mennyi=mysql_num_rows($lekerdez);
			if($mennyi>0) {
				include_once("$design_url/hirlista.php");
				while(list($id,$cim,$intro,$datum)=mysql_fetch_row($lekerdez)) {
					$datido=mktime(substr($datum,11,2),substr($datum,14,2),0,substr($datum,5,2),substr($datum,8,2),substr($datum,0,4));
					$ev=substr($datum,0,4);
					$ora=date('G',$datido);
					$perc=date('i',$datido);
					...
					$honap=alapnyelv("ho$ho");
					$nap=date('j',$datido);
					$datumkiir="$ev $honap $nap. $ora:$perc";
				...
				$adatT[2]=$kod_lista;
				$tipus='hirlistadoboz';
				$kapcsolodo.=formazo($adatT,$tipus);
			}
		}	
		$feltetelT='';
		$feltetel='';
	...
	*/
		...
		list($rovatcim)=mysql_fetch_row(mysql_db_query($db_name,"select nev from rovatkat where id='$rovat'"));
		$rovatid=$rovat;
	}
	else {
		$rovatcim="HIBA";
...
function hirek_kereso() {
	...
	$query1="select id,nev from rovatkat where rovat>0 order by sorszam"; //Főkategóriák
...
//Lista
	...
	if($mennyi==0) {
		...
		if($min>0) {
			...
			$kod_lista.="\n<input type=submit value=Előző class=urlap><input type=text name=leptet value=$leptet class=urlap size=2></form>";
		}
		else {
			$kod_lista.='&nbsp;';
		}
		...
		if($mennyi>$next) {
			...
			$kod_lista.="\n<input type=submit value=Következő class=urlap><input type=text name=leptet value=$leptet class=urlap size=2></form>";
		}
		else $kod_lista.='&nbsp;';
