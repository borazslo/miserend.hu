...
function feltoltes_index() {
    ...
    if (mysql_num_rows($lekerdez) > 0) {
        ...
        while (list($tid, $tnev, $tvaros, $teszrevetel, $megbizhato) = mysql_fetch_row($lekerdez)) {
            ...
            if ($megbizhato == 'i') {
                if ($teszrevetel == 'i')
                    $jelzes.="<a href=\"javascript:OpenScrollWindow('naplo.php?id=$tid',550,500);\"><img src=img/csomag.gif title='Új észrevételt írtak hozzá!' align=absmiddle border=0></a> ";
                elseif ($teszrevetel == 'f')
                    $jelzes.="<a href=\"javascript:OpenScrollWindow('naplo.php?id=$tid',550,500);\"><img src=img/csomagf.gif title='Észrevétel javítása folyamatban!' align=absmiddle border=0></a> ";
                elseif ($vaneszrevetelT['templomok'][$tid])
                    $jelzes.="<a href=\"javascript:OpenScrollWindow('naplo.php?id=$tid',550,500);\"><img src=img/csomag1.gif title='Észrevételek!' align=absmiddle border=0></a> ";
            }
