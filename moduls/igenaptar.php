...
function haview() { 
		...
		while($eredmeny = mysql_fetch_array($_szin)) {
			...
			if(is_array($szentidT)) {
				...
				foreach($szentidT as $kulcs=>$ertek) {		
					if($msz>0) $megszentek.='<span class=kislink>, </span>';
					...
					else $megszentek.='<span class=kislink>';
		...
		if(is_array($igeT)) {
			...
			while(list($id,$ev,$idoszak,$nap,$unnep)=mysql_fetch_row($lekerdezg)) {
				...
				if(!empty($unnep)) $igenap="$igenap$unnep";
				$unnepT[$n]=$igenap;
			}
		}
...
//	if(!empty($ev)) $igenap.="$ev év, ";
	...
	else $igenap="<br><span class=alcim>$igenap</span>";      // dölt betût kivettem .
	...
	if(is_array($szentidT)) {
		foreach($szentidT as $kulcs=>$ertek) {			
			if($a>0) $megszentek.='<span class=kozepeslink>, </span>';
			if(!empty($introT[$kulcs]) or !empty($leirasT[$kulcs])) {
				$link="<a href=?m_id=1&m_op=szview&id=$ertek&szin=$_GET[szin]$linkveg class=kozepeslink>";
			}
			else $megszentek.='<span class=kozepeslink>';
			$megszentek.=$link.$szentnevT[$kulcs];
			if(!empty($link)) $megszentek.='</a>';
			else $megszentek.='</span>';
			$link='';
	...
	$tartalom.="$unnepkiir<br><a href=?m_id=7&m_op=view&id=7$linkveg class=kismenulink title='bõvebben a színekrõl'><small>A nap liturgikus színe: <b>$szinkiir</b></small></a></div>";
	...
	if($van_o or $van_u or $van_e) {
		...
		if($van_o) $tartalom.="<div class=alapkizart><b>$oszov_hely</b> $oszov_biblia<br><br>$oszov</div>";
		if($van_o and $van_u) $tartalom.='<hr>';
		if($van_u) $tartalom.="<div class=alapkizart><b>$ujszov_hely</b> $ujszov_biblia<br><br>$ujszov</div>";
		if($van_e and ($van_o or $van_u)) $tartalom.='<hr>';
		if($van_e) $tartalom.="<div class=alapkizart><b>$evang_hely</b> $evang_biblia<br><br>$evang</div>";
...
function igenaptar_gview() {
	...
	if(!empty($igenap) and !empty($unnep)) $igenap='<br>'.$igenap;
