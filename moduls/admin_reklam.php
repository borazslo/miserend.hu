...
function reklam_add($rid) {
	...
	$urlap.="\n<span class=kiscim>Cím: </span><span class=alap><br>Csak az adminban jelenik meg, a kereshetőség miatt.</span><br><input type=text name=cim value='$cim' class=urlap size=60 maxlength=100>";
	$urlap.="\n<br><br><span class=kiscim>Képfelirat: </span><span class=alap><br>(nem kötelező)</span><br><input type=text name=title value='$title' class=urlap size=60 maxlength=100>";
	$urlap.="\n<br><br><span class=kiscim>URL: </span><span class=alap><br>Klikkelésre hova menjen. Ha más weboldal, akkor http:// -rel kezdődjön a hivatkozás, <br>ha oldalon belüli link, akkor ez nem kell!</span><br><input type=text name=url value='$url' class=urlap size=60 maxlength=100>";
	...
	$urlap.="\n<br><br><span class=kiscim>Sorszám:</span><span class=alap><br>Ha van, akkor sorrendben mindenképp megjelenik, ha nincs, akkor előzőek után véletlenszerűen maximum 5 db (csak alul érvényes)</span><br><input type=text name=sorszam class=urlap maxlength=2 value='$sorszam' size=2>";
	...
	$holnev=array('','Főreklám (jobb fent)','Logó reklámok (jobb lent)');
	for($i=1;$i<=2;$i++) {
...
function reklam_mod() {
	...
	while(list($rid,$cim,$hol)=mysql_fetch_row($lekerdez)) {
		if($hol!=$holell) {
			if($hol==1) $kiir.="<span class=kiscim>-----------------Felső reklámok----------------------</span><br><br>";
...
function reklam_del() {
	...
	$kiir="\n<span class=kiscim>Biztosan törölni akarod a következő reklámot?</span>";
