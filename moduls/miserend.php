  ...
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
	...
	if($mennyi>0) {
		foreach($results['results'] as $templom) {
			...
			if($user->checkRole('miserend') OR $letrehozta==$user->login ) $tartalom.=" <a href=?m_id=27&m_op=addtemplom&tid=$tid$linkveg><img src=img/edit.gif title='szerkesztés' align=absmiddle border=0></a> <a href=?m_id=27&m_op=addmise&tid=$tid$linkveg><img src=img/mise_edit.gif align=absmiddle border=0 title='mise módosítása'></a>";
	...
	*/
	...
	if($mennyi==0) {	
		...
		foreach($results['churches'] as $result) {
			...
			if($user->checkRole('miserend') OR $result['letrehozta']==$user->login) $tartalom.=" <a href=?m_id=27&m_op=addtemplom&tid=".$result['tid']."$linkveg><img src=img/edit.gif title='szerkesztés' align=absmiddle border=0></a>  <a href=?m_id=27&m_op=addmise&tid=".$result['tid']."$linkveg><img src=img/mise_edit.gif align=absmiddle border=0 title='mise módosítása'></a>";
